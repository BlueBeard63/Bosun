...
// e.g. `docker run -p 4222:4222 nats` then BOSUN_NATS_URL=nats://localhost:4222.
func newBus(t *testing.T) *eventnatsmod.Bus {
	t.Helper()
	url := os.Getenv("BOSUN_NATS_URL")
	if url == "" {
		t.Skip("set BOSUN_NATS_URL to run the NATS integration tests")
	}
	b := &eventnatsmod.Bus{Opts: &eventnatsmod.Options{URL: url}}
